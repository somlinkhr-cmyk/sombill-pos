...
-- 003: Core Tables
-- ============================================================================
-- This migration creates all database tables for the multi-tenant restaurant
-- management system.
--
-- Dependencies: 001_extensions.sql (for uuid_generate_v4)
-- Idempotent: Yes (uses CREATE TABLE IF NOT EXISTS)
-- ============================================================================

-- ============================================================================
-- MOCK AUTH SCHEMA (for local testing only)
-- ============================================================================
-- The auth schema exists in Supabase but not in standard PostgreSQL.
-- Create a mock auth schema for local testing compatibility.
-- Skip this on Supabase where auth schema already exists.
DO $$
BEGIN
  -- Only create mock auth schema if it doesn't exist (local PostgreSQL)
  IF NOT EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = 'auth') THEN
    CREATE SCHEMA auth;
    
    -- Create mock auth.users table for local testing
    ...
    -- Create mock auth.uid() function for local testing (Supabase compatibility)
    CREATE FUNCTION auth.uid()
    RETURNS UUID
    LANGUAGE sql
    SECURITY DEFINER
    AS $_$
      SELECT NULL::UUID; -- Returns NULL in local testing, actual user ID in Supabase
    $_$;
    
    RAISE NOTICE 'Created mock auth schema for local testing';
  ELSE
    RAISE NOTICE 'Auth schema already exists (running on Supabase), skipping mock creation';
  END IF;
END $$;
